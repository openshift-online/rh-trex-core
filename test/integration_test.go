...
func TestCoreDinosaurResource(t *testing.T) {
	...
	// Migrate the schema
	// Create a test model with the core API Meta struct
	type TestDinosaur struct {
		api.Meta
		Species string `json:"species"`
	}

	if err := db.AutoMigrate(&TestDinosaur{}); err != nil {
		t.Fatalf("Failed to migrate schema: %v", err)
	}

	// Create core DAO
	dao := coredao.NewBaseDAO[TestDinosaur](db)

	// Create mock event emitter
	eventEmitter := &MockEventEmitter{}

	// Create core service
	service := coreservices.NewBaseCRUDService[TestDinosaur](dao, eventEmitter, "Dinosaurs")

	ctx := context.Background()

	// Test Create
	newDinosaur := &TestDinosaur{
		Meta: api.Meta{
			ID: "test-dino-1",
		},
		Species: "T-Rex",
